# *** EXTRACTING HISTORICAL GRIDMET DATA***
# AUTHOR: HEIDI NGUYEN 
...
var=inputs[7]
cat('\n\n Extracting GridMET historical',var,'\n')
...
outname <- paste0(interimpath,'/interim_gridmet_hist_',var)
if (file.exists(paste0(outname,'.bmat')) & file.exists(paste0(outname,'.desc.txt'))) {
  cat('\nFile already exists\n')
} else {
  cat("\nStart extracting data\n")
  files=list.files(path=rawpath,pattern = var, 
                   full.names = T) 
  ...
  matrix.var=foreach(i = files,.combine = rbind,.packages=c('ncdf4')) %dopar% {
                       filename=paste0(i)
                       nc=nc_open(filename)
                       varid=names(nc$var)
