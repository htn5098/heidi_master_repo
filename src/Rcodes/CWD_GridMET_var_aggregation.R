...
# Input data
data = fm.load(paste0(interimpath,'/interim_gridmet_hist_',var))
...
opfilename=paste0('./data/processed/GridMET_hist_',var,'_county.csv')
if (file.exists(opfilename)) {
  ...
  #clusterExport(cl,list('aggr_data','pr','spfile')) #exporting data into clusters for parallel processing
