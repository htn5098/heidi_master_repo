...
ncfiles <- list.files(path = inputpath, full.names = T) # changing path to gridMET files
...
foreach(i = ncfiles,.verbose=F,.errorhandling='remove') %dopar% { 
  ...
  write.csv(dt.df,paste0(interim,'gridMET_',varatt$standard_name,'.csv'),row.names=F)
}
